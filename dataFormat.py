...
class CallMethods:
    ...
    async def get_description(self):
        ...
        system_prompt = '''Ты Front-end специалист топ уровня, твоя задача редактирование описания товаров в формате 
        html. Ты внимательно относишься к структуре DOM и следуешь стандартам разметки. В ответе не пиши ничего 
        лишнего кроме результата работы в формате html!. Не пиши "```html" в ответе, ответ должен быть на русском 
        языке!.'''
        self.new_description = re.sub('\n', '', await self.call(system_prompt, user_prompt))
        return self.new_description

    async def get_h2(self):
        user_prompt = f'''Описание "{self.new_description}" {PromptData.h2_prompt}'''
        system_prompt = '''Ты Front-end специалист топ уровня, твоя задача редактирование описания товаров в формате 
        html. Ти внимателено относишся к структуре DOM, количеству символов и следуешь стандартам разметки. В ответе 
        не пиши ничего лишнего кроме результата работы в формате html. Не пиши "```html" в ответе, ответ должен быть 
        на русском языке!'''
        return await self.call(system_prompt, user_prompt)

    async def get_faq(self):
        user_prompt = f'''{PromptData.icp}
    Информация о товаре: {self.new_description}
    ...
    async def get_other(self):
        user_prompt: str = f'''Описание: {self.new_description}
    {PromptData.other_data_prompt}'''
        system_prompt = '''Ты SEO специалист топ уровня, твоя задача писать продающий SEO оптимизированый текст для 
        товаров. Ти внимателено относишся к количеству символов и следуешь стандартам SEO. В ответе не пиши ничего 
        лишнего кроме результата работы он должен быть прагматичным и содержательным, и не должен звучать как 
        реклама, ответ должен быть исключительно на русском языке'''
        return await self.call(system_prompt, user_prompt)
...
class ProductBuilder(Builder):
    ...
    def set_type(self, _type="simple"):
        self._product.add(key='type', value=_type)
        ...
        "value": [
            {
                ...
                "content": "<div class='widget-title'>Доставка осуществляется с помощью курьерских "
                           "служб:</div>\r\n<p>• Новая Почта • Delivery</p>\r\n<div class='widget-title'>Нова "
                           "пошта</div>\r\n<p>При доставке товара компанией проводится обязательное страхование "
                           "груза. Не забывайте проверять заказ при получении. После получения магазин не принимает "
                           "претензии относительно внешнего вида и комплектации изделия. Бесплатная доставка от 1500 "
                           "грн. в пределах Украины (бесплатная доставка не распространяется на мебель). При "
                           "оформлении заказа на мебель до 3000 грн. необходимо внесение предоплаты в размере 300 "
                           "грн. При оформлении заказа на мебель от 3000 грн. необходимо внесение предоплаты в "
                           "размере 700 грн. Заказы, стоимостью товара до 200 грн., оформляются только по предоплате. "
                           "Отправка заказов по Украине осуществляется транспортной компанией Новая Почта. "
                           "Крупногабаритные товары (мебель) отправляются компаниями Новая Почта на грузовые "
                           "отделения и 'Delivery'. Оформление заказов с отправкой в Почтоматы ПриватБанка только по "
                           "предоплате.</p>\r\n<div class='widget-title'>Delivery</div>\r\n<p>Данный вид доставки "
                           "рекомендуется для крупногабаритных товаров. При доставке товара транспортной компанией "
                           "проводится обязательное страхование груза на полную стоимость товара. Не забывайте "
                           "проверять внешний вид и комплектацию заказа, когда Вы получаете его в транспортной "
                           "компании. После получения товара магазин не принимает претензии относительно внешнего "
                           "вида и комплектации изделия.</p>\r\n<div class='widget-title'>Способы оплаты</div>\r\n<ul "
                           "class='cs-delivery-info__list'>\r\n<li class='cs-delivery-info__item'><span "
                           "class='cs-delivery-info__caption'><span class='cs-delivery-info__caption'>Наложенный "
                           "платеж Оплата при получении наложенным платежом. Должны уведомить Вас, что Новая Почта "
                           "кроме стоимости доставки берет еще комиссию за наложенный платеж 20 грн. + 2% от "
                           "стоимости заказа. </span></span></li>\r\n<li><span "
                           "class='cs-delivery-info__caption'>Карта ПриватБанка </span><span "
                           "class='cs-delivery-info__comment h-pre-wrap'>Номер карты для оплаты мы сообщим вам в СМС "
                           "или Viber после подтверждения и согласования заказа.</span></li>\r\n<li "
                           "class='cs-delivery-info__item'><span class='cs-delivery-info__caption'>Безналичный расчет "
                           "Оплата на расчетный счёт</span></li>\r\n</ul>"
            }
        ]
    },
        {
